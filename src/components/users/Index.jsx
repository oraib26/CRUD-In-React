    ...
    {users.length >0?users.map((user,index)=>
      (
        ...
        <tr>
          ...
          <td>{user.email}</td>
          <td>{user.password}</td>
          <td className='btn text-danger btn-light border ' onClick={ () => deleteUser(user._id)}>delete</td>
          <td className=' btn btn-light border '><Link className='text-success' to={`/user/edit/${user._id}`}>edit</Link></td>
          <td className=' btn btn-light border '><Link className='text-info' to={`/user/${user._id}`}>details</Link></td>
        </tr>
        </React.Fragment>  
      )
    ):<h2>no user data</h2>}
